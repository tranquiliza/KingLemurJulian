...
namespace KingLemurJulian.Core.Commands
{
    public class Mil2KmCommand : CommandExecutorBase
    {
        ...
        public override async Task Execute(CommandRequest commandRequest)
        {
            ...
            await mediator.Send(new ChatResponseRequest(commandRequest, $"{number} norwegian mil converts to {result.ToInvarientStringWith2Decimals()} kilometers")).ConfigureAwait(false);
        }
    }
}
