...
namespace KingLemurJulian.Core.Commands
{
    public class Km2MilesCommand : CommandExecutorBase
    {
        ...
        public override async Task Execute(CommandRequest commandRequest)
        {
            ...
            await mediator.Send(new ChatResponseRequest(commandRequest, $"{number} kilometers converts to {result.ToInvarientStringWith2Decimals()} miles")).ConfigureAwait(false);
        }
    }
}
