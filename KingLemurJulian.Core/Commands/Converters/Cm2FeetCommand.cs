...
namespace KingLemurJulian.Core.Commands
{
    public class Cm2FeetCommand : CommandExecutorBase
    {
        ...
        {
            var input = command.Argument;
            if (!double.TryParse(input, out var centimeters))
            {
                await mediator.Send(new ChatResponseRequest(command, "Please give me a number")).ConfigureAwait(false);
                return;
            }
            ...
            await mediator.Send(new ChatResponseRequest(command, $"{command.Argument}cm converts to {feet}'{leftOverInches.ToInvarientStringWith2Decimals()}\"")).ConfigureAwait(false);
        }
    }
}
