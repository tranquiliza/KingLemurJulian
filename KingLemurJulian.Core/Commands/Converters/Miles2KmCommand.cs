...
namespace KingLemurJulian.Core.Commands
{
    public class Miles2KmCommand : CommandExecutorBase
    {
        ...
        public override async Task Execute(CommandRequest commandRequest)
        {
            ...
            await mediator.Send(new ChatResponseRequest(commandRequest, $"{number} miles converts to {result.ToInvarientStringWith2Decimals()} kilometers")).ConfigureAwait(false);
        }
    }
}
